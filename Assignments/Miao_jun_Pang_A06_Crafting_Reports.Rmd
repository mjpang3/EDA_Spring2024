...
epa_data1 <- read.csv(
  here("Processed_KEYn","EPAair_O3_PM25_NC1819_Processed.csv"),
...
knitr::kable(summary_PM25, caption = "Mean PM 2.5", col.names = c("County","$\\mu g/m^3$"))
...
ozone_2018 <- epa_data1 %>%
  filter(Year %in% 2018) %>%
  ggplot(aes(x = as.factor(Month), y = Ozone)) +
  geom_boxplot() +
  xlab("Month") +
  ylab("Ozone µg/m3")+
  ggtitle("Monthly Ozone 2018")
...
ozone_2019 <- epa_data1 %>%
  filter(Year %in% 2019) %>%
  ggplot(aes(x = as.factor(Month), y = Ozone)) +
  geom_boxplot() +
  xlab("Month") +
  ylab("Ozone µg/m3")+
  ggtitle("Monthly Ozone 2019")
...
Add a table of contents and list of figures to yoour document and knit to a PDF. Submit your PDF to Sakai, but also be sure to commit and push your Rmd file used to create this knit document to GitHub. In the section below, add a link to your GitHub repository. 

## Git Repository
https://github.com/mjpang3/EDA_Spring2024
